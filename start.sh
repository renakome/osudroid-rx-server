...
install_python_deps() {
    ...
    # Clear pip cache and install corrected osudroid_api_wrapper
    echo "🧹 Clearing pip cache..."
    pip cache purge || true
    echo "🎮 Installing osudroid_api_wrapper..."
    pip install --no-cache-dir git+https://github.com/unclem2/osudroid-api-wrapper.git || {
...
main() {
    ...
    # Install Rust if needed (only if not already in path or home)
    if [ ! -f "$HOME/.cargo/bin/rustc" ] && ! command_exists rustc; then
        install_rust
    else
